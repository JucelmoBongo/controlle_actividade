...
// dando um nome unico no arquivo(foto)
     ...
     $enviado = move_uploaded_file($arquivo['tmp_name'], $pasta . $novoNomeDoArquivo . "."  . $extensao);
    



//     verificando se o email exite
    $emailExite = $conectar->query("SELECT * FROM estudante WHERE email = '$email'");
...
<?php
     ...
     if ($telefone == 9) {
          ...
          $conectar->query("INSERT INTO professor (id_disciplina,nome_professor,email,senha,endereco,foto,sexo,telefone) VALUES ('$disciplina','$nome_professor','$email','$senha','$endereco','$enviado','$genero','$telefone')");
          header("location:listar_professor.php");
     }
     else{
          ?>
          <script> 
          alert(" O numero de telefone tem que ter 9 digitos.");
          </script>
      <?php
          ...
          <div class="mae">
               <div class="filho">
                    ...
                    <form action="#" enctype="multipart/form-data" method="post">
                         <input type="text" name="nome_professor" placeholder="Nome completo" pattern="[a-z A-Z ÁÉÍÓÕÔÃÚÇ áéíóúõôãç]+$" required>
                    ...
                    <div class="disciplina-grupo">
                         ...
                         <div class="disciplina-input" style="border: 1px solid black;border-radius: 8px;">
                              ...
                              <?php
                              // fazendo uma consulta no banco de dados na tabela curso
                                   $query = $conectar->query("SELECT * FROM cursos");
                              ...
                              foreach ($registro as $opcao){
                                   ...
                                   ?>
                                   <option name="curso" value="<?=$opcao['id_curso']?>"><?=$opcao['nome_curso']?></option>
                                   <?php
                                   }
                                   ?> 
                                   </select>
