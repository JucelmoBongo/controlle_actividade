...
if(!isset($_SESSION['nome_adm'])){
  ...
  if(isset($_POST['nome'])){
    ...
    $conectar->query("INSERT INTO actividade_geral (nome,data_realizar) VALUES('$nome','$data')");

  }

$resul = $conectar->query("SELECT * FROM actividade_geral ");
...
<head>
    ...
    <title>Painel Administrador - <?=$_SESSION['nome_adm']?></title>
</head>
<body>
<section class="body-section">
    ...
    <div class="container">
            <div class="page-header">
                <h1 style="padding-top:5%;padding-left:2%;padding-bottom:2%">Actividades Geral</h1>
            </div>
           <table class="table table-striped table-hover "> 
                ...
                <?php
                    ...
                    <tbody>
                        <tr>
                            <td ><?=$row->id_actividade_geral?></td>
                            <td><?=$row->nome?></td>
                            <td><?=$row->data_insercao?></td>
                            <td><?=$row->data_realizar?></td>
                            <td><a href="apagar_actividade.php?id=<?=$row->id_actividade_geral?>" class="btn btn-danger btn-lg btn-sm btn-xr color-white" title="Apagar curso">Apagar</a></td>
           ...
           <div class="card-modal">
            <button style="margin-left:5%;padding-bottom:2px" type="button" class="btn btn-primary botao" id="btn-modal" data-toggle="modal" data-target="#exampleModalCenter">
                ...
                <p class="card-aside-modal ">Adicionar Actividade</p>
            </button>
                <div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                                <form action="#" class="modal-content" method="POST">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalCenterTitle">Adicionar Nova actividade</h5>
                                    ...
                                    <div class="modal-body">
                                        <div class="Texto-modal">
                                            Porfavor insira um nome para a nova actividade...
                                        </div>
                                        <div class="form-modal-body">
                                            <div class="form-modal-body-div">
                                                    <input required class="form-modal-Input"  type="text" name="nome" id="Função" placeholder="Nome..." pattern="[a-z A-Z áéíóú ÁÉÍÓÚ  ãõ ÃÕ ÂÔ çÇ]+$" title="Apenas texto" autofocus>
                                            </div>
                                        
                                        </div>
                                    </div>
                                     <label class="Texto-modal">Data da Realização</label>
                         <div class="col-sm-8">
                                <input type="date" name="data" requerid>
                         </div>
